...
\titleformat{\section}
    {\color{gray!70}\titlerule\color{black}\normalfont\Large\bfseries}
    {\thesection}
    {0.6em}
    {}
...
\renewcommand{\columnseprulecolor}{\color{gray!60}}
\begin{multicols*}{3}
...
$$I_{D} = \begin{cases} 
    \frac{1}{2} \mu_n C_{ox} \frac{W}{L} V_{OV}^2 \cdot {\color{red}(1 + \lambda V_{DS})} & \text{NMOS} \\
    \frac{1}{2} \mu_p C_{ox} \frac{W}{L} |V_{OV}|^2 \cdot {\color{red}(1 + \lambda |V_{DS}|)} & \text{PMOS}
\end{cases}$$
...
Gleichtakt: $A_{CM} = \frac{V_{out}}{V_{CM}} \approx \frac{-g_m R_D}{1 + g_m 2R_{SS}} \approx -\frac{R_D}{2 R_{SS}}$ \quad ($V_{in1} = V_{in2} = V_{CM}$)
...
\begin{itemize}
    \item Verhält sich wie Cascode CM.
    \item $V_\text{ov}, V_{GS}$ über $I_D$ berechnen.
    \item $V_{DS,REF} = V_{DS2} = 2 \cdot V_{GS}, V_{DS,OUT} = V_{DS1} = V_{GS}$
    \item Wenn CLM relevant: $\frac{I_{OUT}}{I_{REF}} = \frac{(W/L)_{OUT}}{(W/L)_{REF}} \cdot \frac{1+\lambda V_{DS,REF}}{1+\lambda V_{DS,OUT}}$
    \item $\implies I_{OUT} = I_{REF} \cdot \frac{(W/L)_{OUT}}{(W/L)_{REF}} \cdot \frac{1+\lambda V_{DS,REF}}{1+\lambda V_{DS,OUT}}$
    \item $\Delta I_{OUT} = |I_{REF} - I_{OUT}|$
    \item $r_0$ wie Kaskode.
\end{itemize}
\algo{Wilson-Stromspiegel mit 4 Transistoren:}

\begin{itemize}
    \item Verhält sich wie Self-Biased Cascode CM. 
    \item $V_{DS1} = V_{DS2} =V_{DS4} = V_{GS}$
    \item $I_{OUT} = I_{REF}$
    \item $V_{OUT_{min}} = V_{TH} + 2V_{OV}$
    \item Stromänderung durch $\Delta U$ verursacht: $\Delta I_{OUT} = \frac{\Delta U}{R_{OUT}}$. Relativ: $\frac{\Delta I_{OUT}}{I_{OUT}}$.
\end{itemize}
...
$$
Z_1 = \frac{Z}{1-A}, \quad Z_2 = \frac{Z}{1-\frac{1}{A}}  \implies C_1 = C \cdot (1-A)
$$
...
\includegraphics[width=0.75\linewidth]{assets/PMB.jpeg}
...
\section{Digital Building Blocks}
...
\item \datastruct{Metropolis-Kriterium}: Wird verwendet, um zu entscheiden, ob eine neue Lösung angenommen wird 
$$
\text{random}[0,1] \leq \min \{1,e^{-\frac{\text{cost}(S_\text{new})- \text{cost}(S)}{T}} \}
$$
\begin{tabbing}
\quad \= \quad \= \quad \= \kill
...
\includegraphics[width=\linewidth]{assets/AU1.jpeg}
\includegraphics[width=\linewidth]{assets/AU2.jpeg}
\includegraphics[width=\linewidth]{assets/AU3.jpeg}
\includegraphics[width=\linewidth]{assets/Bode.jpeg}
