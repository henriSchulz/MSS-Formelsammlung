...
\algo{Sättigungsbereich ($|V_DS| \ge |V_OV|$):} \\
$$I_{D} = \begin{cases} 
    \frac{1}{2} \mu_n C_{ox} \frac{W}{L} V_{OV}^2 & \text{NMOS} \\
    \frac{1}{2} \mu_p C_{ox} \frac{W}{L} |V_{OV}|^2 & \text{PMOS}
\end{cases}$$
\algo{Transkonduktanz ($g_m$):} \\
$$g_m = \left . \frac{\partial I_D}{\partial V_{GS}}\right|_{V_{DS}=\text{const.}} = \mu \cdot C_{ox} \frac{W}{L} V_{\text{Ov}} = \sqrt{2 \cdot \mu \cdot C_{ox} \cdot \frac{W}{L} \cdot I_D}$$
...
$$g_{mb} = \frac{\partial I_D}{\partial V_{BS}} = \m_n \cdot C_{ox} \frac{W}{L} (V_{GS} - V_{Th})\left(-\frac{\partial V_{Th}}{\partial V_{BS}}\right) =  \eta \cdot g_m$$
...
\end{itemize}
\includegraphics[width=0.5\linewidth]{assets/fcan.png}
...
\begin{itemize}
    \item \textbf{M1 (Eingang):} Dominanter Beitrag. $\overline{v_{n,in,M1}^2} \approx \frac{4kT\gamma}{g_{m1}}$
    \item \textbf{M2 (Kaskode):} \textbf{Vernachlässigbar}, da der Rauschstrom von M2 durch den hohen Ausgangswiderstand $r_{o1}$ von M1 unterdrückt wird.
    \item \textbf{Last:} Trägt signifikant bei.
\end{itemize}

$$\overline{i_{n,out,total}^2} = \overline{i_{n,in,M1}^2} + \overline{i_{n,in,Load}^2}$$
$$\overline{v_{n,out,total}^2} = \overline{i_{n,out,totel}^2} \cdot R_{out}^2$$
$$\overline{v_{n,in,Load}^2} = \frac{\overline{i_{n,out,totel}^2}}{g_{m1}^2}$$
...
\algo{Verstärkung} \\
Gegen: $|A_d| = |\frac{V_{out}}{V_{id}}| = g_m \cdot (R_D || r_o)$ \\
\textcolor{red}{bei aktiver Last: $r_{o1/2} || r_{o3/4}$} \\
Gleich: $|A_{CM}| = |\frac{V_{out}}{V_{CM}}| \approx \frac{R_D}{2 R_{SS}}$ \quad ($V_{in1} = V_{in2} = V_{CM}$)
...
$$
H(s) = \left. \frac{(1 + \frac{s}{\omega_z_1})(1 + \frac{s}{\omega_z_2})...(1 + \frac{s}{\omega_z_n})}{(1 + \frac{s}{\omega_p_1})(1 + \frac{s}{\omega_p_2})...(1 + \frac{s}{\omega_p_m})} \right|_{s = j\omega}
$$
\begin{itemize}
...
\begin{align*}
  R_{d1} &= r_{o1} \| R_{\text{in2}} \\
  R_{\text{in2}} &= \frac{r_{o2} + R_L}{1 + g_{m2} r_{o2}} \\
  R_o &= g_{m2} r_{o2} r_{o1} + r_{o1} + r_{o2} \\

  \tau_1 = R_{C_{gs1}} \cdot C_{gs1}
  &= (R_{\text{sig}}) \cdot C_{gs1} \\

  \tau_2 = R_{C_{gd1}} \cdot C_{gd1}
  &= \frac{v_x}{i_x} \cdot C_{gd1}
  = \left( g_{m1} R_{d1} R_{\text{sig}} + R_{\text{sig}} + R_{d1} \right) \cdot C_{gd1} \\

  R_{d1} = r_{o1} \| R_{\text{in2}} &= r_{o1} \| \frac{r_{o2} + R_L}{1 + g_{m2} r_{o2}} \quad \text{Vgl. CG: } R_{C_{gs}} \\

  \tau_3 = R_{C_{d1}} \cdot (C_{db1} + C_{gs2})
  &= (R_{d1}) \cdot (C_{db1} + C_{gs2}) \\

  R_o \approx g_{m2} r_{o2} r_{o1} \quad \text{Vgl. Kaskode Ausgangswiederstand} \\

  \tau_4 = R_{C_{out}} \cdot (C_L + C_{gd2} + C_{db2})
  &= (R_o \| R_L) \cdot (C_L + C_{gd2} + C_{db2})
\end{align*}
...
\begin{itemize}
    ...
    \item $T = -\frac{i_\text{returned}}{i_\text{test}} = -\frac{v_\text{gs}(i_\test{test})}{i_\text{test}}$
\end{itemize}    
...
\hline
\textbf{Modell} & \textbf{Beschreibung / Charakteristik} & \textbf{Hauptvorteil} \\ \hline
